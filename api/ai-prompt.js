...
2. definition(정의): 50자 이내. 패턴 "X를 위해, [핵심키워드 2개 이상 활용]한 Y(명사)". 한국어. 명사로 끝남.
3. mnemonic(내용): 두음·구성요소·약어 등 외우기 쉬운 핵심요약. 줄바꿈으로 구분. 5줄 이내.
4. keyword(키워드): 핵심 기술요소 5개를 쉼표로 구분.
5. references(참고): 권위 있는 출처 1~3개 (https://blog.skby.net/, Wikipedia 등). 실재하는 한국어 URL 만 (학습 시점 knowledge 기준).
...
규칙:
1. 분량: 50자 이내 (공백 포함). 50자 절대 초과 금지.
2. 패턴: "X를 위해, [핵심키워드 2개 이상 활용]한 Y(명사)"
...
예시:
- 토픽: Zero Trust → "내부망 침해 차단 위해, 000,0000 기술을 통한 모든 접근 검증한 보안모델"
